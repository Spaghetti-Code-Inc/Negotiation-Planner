...
import 'package:negotiation_tracker/multi_thumb_slider/src/thumb_overdrag_behaviour.dart';
import 'default_thumb.dart';
import 'multi_thumb_slider.dart';
import 'multi_thumb_slider_controller.dart';
...
class MultiThumbSliderState extends State<MultiThumbSlider> {
  ...
  void _handleDragUpdate(int index, DragUpdateDetails details) {
    // I CHANGED: SHOULD BE (&& index == 0)
    if (widget.lockBehaviour.isStartLocked) return;
    if (index == valueListeners.length - 1 &&
        widget.lockBehaviour.isEndLocked) {
      return;
    }
          ...
          ...valueListeners.mapIndexed((index, valueListener) {
            final handle = GestureDetector(
                ...
                child: thumbBuilder != null
                    ? thumbBuilder.call(context, index, valueListener.value)
                    : const DefaultThumb());
            return Positioned.fill(
              child: AnimatedBuilder(
                ...
                builder: (context, child) {
                  return Align(
                    alignment: Alignment(valueListener.value*2-1+.025, .5),
                    child: IgnorePointer(
